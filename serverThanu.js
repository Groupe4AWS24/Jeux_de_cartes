...
function setupSocket(server) {
  ...
  const rooms = {}; 
  const playerDetails = {};
  const temporaryDisconnects = {}; // Liste globale pour gérer les déconnexions
  const DISCONNECT_TIMEOUT = 60000; // Durée de temporisation de déconnexion en millisecondes (60 secondes)
  const MAX_DISCONNECT_TURNS = 2;
...
function handleReconnect(username, socket) {//fonction revisée 
   // Rechercher le joueur dans la liste des déconnexions temporaires
  ...
  /**
 * Gère la déconnexion d'un joueur. Cette fonction prend en charge les scénarios de déconnexion volontaire
 * et involontaire. Elle assure la mise à jour de la salle et informe tous les joueurs de manière appropriée.
 * Si la salle est vide suite à cette action, elle est également supprimée.
 * 
 * @param {string} socketId - L'identifiant du socket qui s'est déconnecté ou qui a abandonné.
 * @param {boolean} isAbandon - Indique si la déconnexion était due à un abandon de la part du joueur.
 */
function handlePlayerDisconnect(socketId, isAbandon) {
  // Récupère les détails du joueur à l'aide de son identifiant de socket.
  const details = playerDetails[socketId];
  if (!details) {
      console.log("Aucun détail trouvé pour le socket :", socketId);
      return;  // Sortie si aucun détail n'est trouvé pour le socket.
  }

  const { roomId } = details;
  const room = rooms[roomId];
  if (!room) {
      console.log("Aucune salle trouvée avec l'ID :", roomId);
      return;  // Sortie si aucune salle n'est associée au joueur.
  }

  // Émet un événement pour notifier les autres joueurs de la salle de la déconnexion ou de l'abandon.
  ...
  // Retire le joueur de la liste des joueurs de la salle.
  room.players = room.players.filter(player => player.socketId !== socketId);

  // Si la salle est désormais vide, la supprime des listes de salles.
  if (room.players.length === 0) {
      console.log(`Suppression de la salle ${roomId} car elle est maintenant vide.`);
      delete rooms[roomId];
  }

  // Supprime les détails du joueur de la liste globale pour nettoyer la mémoire.
  delete playerDetails[socketId];

  // Si le joueur a abandonné le jeu, déconnecte son socket.
  if (isAbandon) {
      const socket = io.sockets.sockets.get(socketId);
      if (socket) {
          console.log(`Déconnexion du socket ${socketId} en raison de l'abandon.`);
          socket.disconnect();
      }
  }
}
